...
.Traducciones
* Francés: http://fr.perl6intro.com
* Alemán:  http://de.perl6intro.com
* Español: http://es.perli6ntro.com
...
<| my $var = 7; ?$var; <| True # un valor Verdadero
...
| ... | Infijo | Constructor de listas perezozas |  0...9999  |  devuelve los elementos si son solicitados
...
* `shell` Ejecuta un comando desde el shell del sistema y dependerá de la plataforma y la shell.
...
`dir` es el equivalente de `ls` en Windows.
...
En este ejemplo, el operador inteligente de coincidencia `~~` se utiliza para comprobar si el texto (iluminación) contiene la palabra (ilumina). +
...
=== Comodines
Se pueden utilizar también comodines en una regex.
...
WARNING: La regex que utilizaremos en este ejemplo para validar una dirección de email no es muy precisa. +
Su propósito es demostrar el funcionamiento de las regex en Perl 6. +
...
Perl 6 permite la creación y redistribución de *módulos*. Cada módulo es un conjunto de funcionalidades empaquetadas que, una vez instalado, se puede reutilizar.

_Panda_ es el gestor de módulos que incorpora Rakudo.

Para instalar un módulo concreto, introduce el siguiente comando en el terminal:
...
Cuando se registra un nuevo usuario, sus credenciales no se guardan en texto plano, se guardan _cifradas_.
La razón de esto es que si un atacante compromete la base de datos no pueda conocer las contraseñas.
...
WARNING: En la práctica el cifrado MD5 solamente no es suficiente pues es vulnerable a ataques de diccionario. +
...
Unicode es un estándar para codificar y representar texto e incluye la mayoría de los sistemas de escritura del mundo. +
...
my @promesa1 = start @array1.map( {is-prime($_ + 1)} );
my @promesa2 = start @array2.map( {is-prime($_ - 1)} );
...
El debate principal de Perl 6 sucede en el canal de IRC link:irc://irc.freenode.net/#perl6[#perl6]. Cualquier consulta al respecto puedes realizarla en: +
http://perl6.org/community/irc

Mantente al día en el siguiente agregador de blogs que tratan de Perl 6: +
http://pl6anet.org/
